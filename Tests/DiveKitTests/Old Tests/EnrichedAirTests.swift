...
final class EnrichedAirTests: XCTestCase {
    ...
    func testMaximumOperatingDepthMinimumSaltwater() throws {
        ...
        try imperialValues.forEach { (percentage, fractionOxygen, maxOp) in
            ...
            let mod = try enrichedAir.maximumOperatingDepth(fractionOxygen: fractionOxygen, gas: gas, decimalPlaces: 0)
        ...
        try metricValues.forEach { (percentage, fractionOxygen, maxOp) in
            let gas = try Gas.enrichedAir(percentage)
            let mod = try enrichedAir.maximumOperatingDepth(fractionOxygen: fractionOxygen, gas: gas, decimalPlaces: 1)
            XCTAssertEqual(mod, maxOp)
        }
    }
    func testThrows() throws {
        ...
        XCTAssertTrue(enrichedAir.exceedsMaximumOperatingDepth(with: gas, fractionOxygen: 1.4, depth: 132))
        XCTAssertFalse(enrichedAir.exceedsMaximumOperatingDepth(with: gas, fractionOxygen: 1.4, depth: 111))
    }
    ...
    func testEquivalentAirDepth() throws {
        ...
        try imperialValues.forEach { (oxygenPercentage, depth, trustedResult) in
            ...
            let ead = try enrichedAir.equivalentAirDepth(depth: depth, gas: gas, decimalPlaces: 0)
    ...
    func testEAD() throws {
        ...
        var ead = try enrichedAir.equivalentAirDepth(depth: 90, gas: gas, decimalPlaces: 0)
        
        XCTAssertEqual(ceil(ead), 67)
        ead = try enrichedAir.equivalentAirDepth(depth: 33, gas: gas, decimalPlaces: 0)
        XCTAssertEqual(ceil(ead), 20)
        enrichedAir = DKEnrichedAir.init(waterType: .saltWater, measurementUnit: .metric)
