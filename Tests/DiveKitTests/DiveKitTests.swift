...
final class DiveKitTests: XCTestCase {
    ...
    func testAtmospheresAbsolute() {
        ...
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 0), 1)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 33), 2)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 66), 3)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 99), 4)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 132), 5)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 87), 3.64)
        // Test Sea Water and Metric
        diveKit = DiveKit.init(waterType: .saltWater, measurementUnit: .metric)
        physics = DKPhysics.init(with: diveKit)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 0), 1)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 10), 2)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 20), 3)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 30), 4)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 40), 5)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 38.5), 4.85)
        physics = DKPhysics.init(waterType: .freshWater, measurementUnit: .imperial)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 34), 2)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 68), 3)
        physics = DKPhysics.init(waterType: .freshWater, measurementUnit: .metric)
        XCTAssertEqual(physics.atmospheresAbsolute(depth: 10.3), 2)
    ...
    func testPartialPressure() {
        ...
        XCTAssertEqual(gasCalculator.partialPressure(of: .air).oxygen, airPartialPressure.oxygen)
        XCTAssertEqual(gasCalculator.partialPressure(of: .air).nitrogen, airPartialPressure.nitrogen)
        XCTAssertEqual(gasCalculator.partialPressure(of: .air).trace, airPartialPressure.trace)
    }
    func testConstants() {
    ...
    func testGasesInititalizers() {
        ...
        XCTAssertEqual(gasCalculator.respiratoryMinuteVolume(gasConsumed: 900, tank: tank, depth: 66, time: 10), 0.8)
        XCTAssertEqual(gasCalculator.surfaceAirConsumption(time: 10, depth: 90, gasConsumed: 600).roundTo(decimalPlaces: 1), 16.1)
